...
On a planté des bulbes de lys à une quinzaine de centimètres dans du terreau (diffusivité thermique de l'ordre de #quan[1e-7 m^2/s]). Pour commencer leur germination, les bulbes ont besoin d'une température d'environ #quan[10 °C].
...
#question(
    ...
    Pour une période $T_"jour" = #quan[24 h]$, la profondeur de peau est $delta_"jour" = sqrt((2 D)/omega) = sqrt((D T_"jour")/pi) = #qty(d1j, "m", chiffres: 1)$.

    À une profondeur de #quan[15 cm], l'onde de température journalière est atténuée d'un facteur $exp(-d/delta) = #num(calc.exp(-d / d1j), chiffres: 1)$. Les variations journalières de température ne se font donc quasiment pas sentir à cette profondeur.

    On modélise la température en surface par $T(0,t) = - A cos(omega t) + T_0$ avec $A = (19-7)/2 = #quan[6 °C]$, $T_0 = (19+7)/2 = #quan[13 °C]$ et $omega = (2 pi)/ T_"année" = #qty((2 * calc.pi) / (365 * 24 * 3600), "rad/s", chiffres: 1)$. L'origine des temps est prise au 1er janvier.
