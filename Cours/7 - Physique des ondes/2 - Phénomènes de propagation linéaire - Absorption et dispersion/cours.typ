...
Cette relation de dispersion n'admet pas de solution réelle. On introduit le nombre d'onde complexe $underline(k)=k_r+j k_i$ solution de la relation de dispersion.

= Ondes progressives en milieu linéaire
== Milieu linéaire
Pour une équation aux dérivées partielles linéaire, si $y_1$ et $y_2$ sont solutions, alors $lambda y_1+mu y_2$ est également solution.
...
Un paquet d'onde est une superposition d'OPH de pulsations proches et d'extension spatiale et temporelle finie.
...
La largeur spectrale du paquet d'onde est reliée à son étendue temporelle.
...
La partie imaginaire de $underline(k)$ est liée à la variation de l'amplitude au cours de la propagation de l'onde.
...
#application[
    Déterminer la profondeur de peau d'une OPPH se propageant dans le sens des $x$ croissants dans un milieu vérifiant l'équation de diffusion.
]
