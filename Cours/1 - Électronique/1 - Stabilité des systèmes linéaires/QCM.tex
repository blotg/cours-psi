...
\element{1.1.3}{
  \begin{questionmult}{stabiliteLaplace}\bareme{haut=2,p=0}
    Parmi les fonctions de transfert ci-dessous, laquelle (lesquelles) décrit(vent) un système stable ? $\tau$ désigne une constante strictement positive.
    \begin{multicols}{2}
      \begin{reponses}
...
\element{1.1.4}{
  \begin{questionmult}{solutionDeltaNegatif}\bareme{haut=3,p=0}
    Parmi les fonctions suivantes, pour lesquelles existe-t-il des valeurs du discriminent du polynôme caractéristique telles qu'elles sont solutions de l'équation différentielle $A\frac{d^2s}{dt^2} + B\frac{ds}{dt} + C s(t) = 0$ ? $A\neq 0$, $B$, $C$, $K$, $K'$ et $K'' \neq 0$ désignent des constantes.
    % \begin{multicols}{2}
      \begin{reponses}
