...
#question(coups-de-pouce: (
  ...
  Un manuel indique : « avec $j = qty("150", "A/m^2")$, la couche atteint $qty("10", "µm")$ en $qty("30", "min")$ ». Est-ce en accord avec l'étude théorique ? Sinon, proposer une explication.
][
  Théorie : $q = 150 times 1800 = qty("2.7e5", "C/m^2")$, $n = 2.7 times 10^5 \/ (6 times 96500) approx qty("0.47", "mol/m^2")$, masse $qty("47.6", "g/m^2")$, volume $qty("1.5e-5", "m^3/m^2")$, soit $e_"théo" approx qty("15", "µm")$.

  Le manuel annonce $qty("10", "µm")$, soit nettement moins. Le rendement faradique n'est donc pas de $qty("100", "%")$ : $rho_F approx 10 \/ 15 approx qty("67", "%")$. Une partie du courant sert au dégagement de #ce("O2") à l'anode (réaction parasite), et une partie de l'alumine formée se dissout dans l'acide sulfurique concentré.
]
