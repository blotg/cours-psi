...
Données : $E^circ(cpl("Pb^2+", "Pb")) = #quan[-0.13 V]$, $E^circ(cpl("Cu^2+", "Cu")) = #quan[0.34 V]$, $E^circ(cpl("Ag+", "Ag")) = #quan[0.80 V]$, $E^circ(ce("O2/H2O")) = #quan[1.23 V]$.
...
#question(coups-de-pouce: (
  ...
  Écrire l'équation-bilan de la réaction. Déterminer sa constante d'équilibre à $#quan[298 K]$. Commenter.
][
  Sur l'échelle des potentiels, #ce("Ag+") (oxydant, $#quan[0.80 V]$) est au-dessus de #ce("Cu") (réducteur, $#quan[0.34 V]$) : #ce("Ag+") oxyde #ce("Cu").
  $ 2 ce("Ag+") + ce("Cu") -> 2 ce("Ag") + ce("Cu^2+") quad (n = 2) $
  $ Delta_r G^circ = - n cal(F) (E^circ(cpl("Ag+", "Ag")) - E^circ(cpl("Cu^2+", "Cu"))) = - 2 times 96500 times 0.46 approx #quan[-89 kJ/mol] $
...
#question(coups-de-pouce: (
  ...
  #strong[Anode] ($E_A$, entre $#quan[0.34 V]$ et $#quan[0.80 V]$) : sont oxydés les métaux dont $E^circ < E_A$, soit
  $ ce("Cu") -> ce("Cu^2+") + 2 e^- quad "et" quad ce("Pb") -> ce("Pb^2+") + 2 e^- $
  L'argent ($E^circ = #quan[0.80 V] > E_A$) n'est pas oxydé : il tombe au fond sous forme de #strong[boues anodiques].

  #strong[Cathode] ($E_C$, un peu en dessous de $#quan[0.34 V]$) : seul #ce("Cu^2+") est réduit,
  $ ce("Cu^2+") + 2 e^- -> ce("Cu") $
  #ce("Pb^2+") ($E^circ = #quan[-0.13 V] < E_C$) n'est pas réduit et reste en solution.
]

#question[
  Expliquer l'intérêt de cette méthode pour purifier le cuivre.
][
  L'argent, non oxydé, est récupéré séparément dans les boues anodiques (sous-produit valorisable). Le plomb, oxydé à l'anode mais non redéposé à la cathode, reste en solution. Le cuivre déposé à la cathode est donc débarrassé de ses deux impuretés : on obtient du cuivre très pur ($approx #quan[99.99 %]$).
]
