...
Le champ magnétique dans un IRM est stationnaire et de l'ordre de #quan[1 T]. La conductivité électrique du corps humain est de l'ordre de #quan[0.2 S/m].

Lors de l'entrée dans la machine, le patient se déplace à une vitesse d'environ #quan[0.2 m/s].
...
#question(coups-de-pouce:(
  ...
  On modélise le corps humain par un cylindre conducteur de rayon $R = #quan[0.2 m]$ et de hauteur $h = #quan[1.7 m]$.
  
  #let pulsation = 2 * calc.pi / 10
  On estime la taille de l'IRM à #quan[1 m]. Le temps d'entrée dans la machine est donc $Delta t = L/v = #quan[5 s]$. Cette durée est la durée nécessaire pour passer du champ magnétique minimal au champ maximal. Pour un signal sinusoïdal, il s'agirait de la demi période. On note donc $T=#quan[10 s]$. La pulsation associée est donc $omega = 2 pi / T = #qty(pulsation, "rad/s", chiffres: 1)$.
  ...
  Cette puissance est négligeable devant la puissance métabolique d'un être humain au repos (environ #quan[100 W]). On en conclut que la chaleur dégagée n'est pas dangereuse.
