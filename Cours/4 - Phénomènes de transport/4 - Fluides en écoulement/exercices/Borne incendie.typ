...
#question(
    ...
    $
        Delta P_2 = #qty(DP2, "Pa", chiffres: 1)
    $
    La perte de charge totale est $Delta P_1 + Delta P_2 = #qty(DP1 + DP2, "Pa", chiffres: 1)$
    ...
    #let Pc = P0 + mv * g * h
    $
      P_"château" = P_0 + mu g h = #qty(Pc, "Pa", chiffres: 2)
    $
    En tenant compte de la perte de charge, la pression disponible au niveau des poteaux d'incendie est donc :
    $
      P_"poteau" = P_"château" - (Delta P_1 + Delta P_2) = #qty(Pc - (DP1 + DP2), "Pa", chiffres: 2)
    $
